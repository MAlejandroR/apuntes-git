...
/**
 * Clase 'BBDD.php_' -> Conexión con la Base de Datos (MySQLi)
 */
...
class BBDD {
     ...
     */
    public function __construct(string $h="localhost", string $u="manuel_hestia", string $p="P@ssw0rd", string $bd="manuel_tienda", int $puerto=3306) {
        $this->host = $h;
        $this->user = $u;
        $this->pass = $p;
        $this->bd = $bd;
    ...
    private function conexion(): mysqli { //: mysqli -> El tipo que devuelve...
        try {
            $con = new mysqli($this->host, $this->user, $this->pass, $this->bd, $this->port);
