...
//var_dump($practica);
if (is_null($practica)) {
    header("index.php_?msj=No se ha leído ninguna prácica");
    exit();
}
...
try {
    ...
    $error .= "Error en correccion.php_ " . $e->getMessage() . "<br />";
...
FIN;
        ...
        break;
    case 'volver':
        header("Location:show.php_");
        exit();
    case "volver a $alumno":
        header("Location:show_individual.php_?posicionAlumnoActual=$posicionAlumnoActual");
        exit();
   case "Escribir en Word":
       ...
       header("Location:writeWord.php_?practica=$practica&alumno=$alumno&comentarios=$comentarios&nota=$nota");
//       header("Location:writeWord.php_?comentarios=$comentarios");
       exit;
