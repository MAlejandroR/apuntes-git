...
function show_files_directory_uploads() {
  ...
  $html .= "<form action='descarga.php_' method='POST' id=f1 >\n";
    ...
    if (($dir != '.') && ($dir !== '..')) {
      ...
      foreach ($ficheros as $fichero) {
        ...
        $html .= "<input type=checkbox name='ficheros_publicar[]' :checked='check_$dir'
                    value='$dir/$fichero' v-model='$dir'  @change='actualizar' />\n";
        $html .= "<a href = ./descargas/uploads/$dir/$fichero>$fichero</a><br />\n";
      }
      if ($contenido) {
        $html .= "<br /><button   v-on:click='select_all_$dir' >{{boton_check_$dir}}</button>\n";
      } else {
