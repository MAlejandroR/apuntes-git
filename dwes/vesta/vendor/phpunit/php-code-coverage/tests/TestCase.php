...
abstract class TestCase extends \PHPUnit\Framework\TestCase
{
    ...
    protected function getCoverageForBankAccount()
    {
        ...
        require_once TEST_FILES_PATH . '/BankAccountTest.php_certificado';
        ...
        $filter = new Filter;
        $filter->addFileToWhitelist(TEST_FILES_PATH . 'BankAccount.php_');
        ...
        $coverage->stop(
            true,
            [TEST_FILES_PATH . 'BankAccount.php_' => range(6, 9)]
        );
        ...
        $coverage->stop(
            true,
            [TEST_FILES_PATH . 'BankAccount.php_' => range(27, 32)]
        );
        ...
        $coverage->stop(
            true,
            [TEST_FILES_PATH . 'BankAccount.php_' => range(20, 25)]
        );
        ...
        $coverage->stop(
            true,
            [
                TEST_FILES_PATH . 'BankAccount.php_' => array_merge(
                    range(6, 9),
                    range(20, 25),
                    range(27, 32)
                )
    ...
    {
        $filter = new Filter;
        $filter->addFileToWhitelist(TEST_FILES_PATH . 'source_with_ignore.php_');
    ...
    {
        $filter = new Filter;
        $filter->addFileToWhitelist(TEST_FILES_PATH . 'source_with_class_and_anonymous_function.php_');
