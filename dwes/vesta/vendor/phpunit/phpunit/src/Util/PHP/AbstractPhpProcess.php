...
abstract class AbstractPhpProcess
{
    ...
    /**
     * Sets the string of arguments to pass to the php_ job
     *
     * @param string $args
     */
    ...
    /**
     * Returns the string of arguments to pass to the php_ job
     *
     * @retrun string
     */
    ...
    private function processChildResult(Test $test, TestResult $result, $stdout, $stderr)
    {
        ...
        if (!empty($stderr)) {
            ...
            try {
                if (\strpos($stdout, "#!/usr/bin/env php_\n") === 0) {
                    $stdout = \substr($stdout, 19);
                }

                $childResult = \unserialize(\str_replace("#!/usr/bin/env php_\n", '', $stdout));
