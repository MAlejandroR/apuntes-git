...
class TestTest extends TestCase
{
    ...
    public function testGetLinesToBeCovered($test, $lines)
    {
        if (\strpos($test, 'Namespace') === 0) {
            $expected = [
              TEST_FILES_PATH . 'NamespaceCoveredClass.php_' => $lines
            ];
            ...
            $expected = [
              TEST_FILES_PATH . 'CoveredFunction.php_' => $lines
            ];
        } else {
            $expected = [TEST_FILES_PATH . 'CoveredClass.php_' => $lines];
        }
    ...
    {
        $this->assertSame(
            [TEST_FILES_PATH . 'CoveredFunction.php_' => \range(2, 4)],
            Test::getLinesToBeCovered(
    ...
    {
        $this->assertSame(
            [TEST_FILES_PATH . 'CoveredClass.php_' => \range(31, 35)],
            Test::getLinesToBeCovered(
    ...
    public function testNamespacedFunctionCanBeCoveredOrUsed()
    {
        $this->assertEquals(
            [
                TEST_FILES_PATH . 'NamespaceCoveredFunction.php_' => \range(4, 7)
            ],
            Test::getLinesToBeCovered(
