...
abstract class AbstractPart extends Word2007AbstractPart
{
    ...
    /**
     * Write fonts faces declaration.
     *
     * @param \PhpOffice\Common\XMLWriter $xmlWriter
     */
    protected function writeFontFaces(XMLWriter $xmlWriter)
    {
        $xmlWriter->startElement('office:fonts-face-decls');
        $fontTable = array();
            ...
            foreach ($styles as $style) {
                ...
                if ($style instanceof Font) {
                    ...
                    if (!in_array($name, $fontTable)) {
                        ...
                        // style:fonts-face
                        $xmlWriter->startElement('style:fonts-face');
                        $xmlWriter->writeAttribute('style:name', $name);
                        $xmlWriter->writeAttribute('svg:fonts-family', $name);
        ...
        if (!in_array(Settings::getDefaultFontName(), $fontTable)) {
            ...
            $xmlWriter->writeAttribute('svg:fonts-family', Settings::getDefaultFontName());
            $xmlWriter->endElement();
        }
        $xmlWriter->endElement();
    }
